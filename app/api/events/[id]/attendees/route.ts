...
import { connectedSql, notBlockedSql } from '@/lib/connections';
...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const rels = ids.length
    ? await query<{
        ...
        shared_history: boolean; shared_music: boolean; visible: boolean;
      }>(
        `select m.id,
  ...
  const rank = (id: string) => {
    ...
    if (!r) return 5;
    if (r.is_connected) return 0;
    if (r.is_friend) return 1;
    if (r.shared_history) return 2;
    if (r.shared_music) return 3;
    return 4;
  };
